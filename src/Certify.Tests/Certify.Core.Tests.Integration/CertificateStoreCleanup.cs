...
namespace Certify.Core.Tests
{
    ...
    {
        [TestMethod]
        public async Task TestCertCleanup()
        {
            ...
            CertificateManager.PerformCertificateStoreCleanup(new DateTime(1936, 06, 01));
            ...
            try
            {
                ...
                // check cert test 4 exists (expired for less than 1 month)
                Assert.IsTrue(CertificateManager.IsCertificateInStore(cert4), "Cert 4 Should Not Be Removed");
            }
            finally
            {
                // clean up after test
