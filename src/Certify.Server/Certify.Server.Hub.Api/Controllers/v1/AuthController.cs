...
namespace Certify.Server.Hub.Api.Controllers
{
    ...
    public partial class AuthController : ApiControllerBase
    {
        ...
        [HttpPost]
        [Route("oidc/debug/token-exchange")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [AllowAnonymous]
        public async Task<IActionResult> DebugTokenExchange(string code, string provider = "default")
        {
            try
            {
                var oidcConfig = await GetOidcProviderWithSecret(provider);
                ...
                var tokenRequest = new Dictionary<string, string>
                {
                    ...
                    ["client_secret"] = oidcConfig.ClientSecret ?? "** NOT SET **",
                    ["code"] = code,
                ...
                _logger.LogInformation("OIDC Debug: Token Exchange - ClientId: {clientId}, RedirectUri: {redirectUri}", oidcConfig.ClientId, oidcConfig.RedirectUri);
                ...
                _logger.LogInformation("OIDC Debug: Token Exchange - Response Status: {status}", response.StatusCode);
                _logger.LogInformation("OIDC Debug: Token Exchange - Response Body: {body}", responseContent);
                ...
                return Ok(new
                {
                    ...
                    requestConfig = new
                    {
                        ...
                        hasClientSecret = !string.IsNullOrEmpty(oidcConfig.ClientSecret)
                    },
            ...
            {
                _logger.LogError(ex, "OIDC Debug: Token exchange failed");
                return BadRequest(new { error = ex.Message, exceptionType = ex.GetType().Name });
            }
        }
        ...
        [HttpGet]
        [Route("oidc/debug/discovery")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [AllowAnonymous]
        public async Task<IActionResult> DebugDiscovery(string provider = "default")
        {
            try
            {
                var oidcConfig = await GetOidcProviderWithSecret(provider);
                ...
                return Ok(new
                {
                    ...
                    signingKeysCount = config.SigningKeys?.Count ?? 0,
            ...
            {
                _logger.LogError(ex, "OIDC Debug: Discovery endpoint retrieval failed");
                return BadRequest(new { error = ex.Message, exceptionType = ex.GetType().Name });
            }
        }
        ...
        [HttpPost]
        [Route("oidc/debug/validate-token")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [AllowAnonymous]
        public async Task<IActionResult> DebugValidateToken(string idToken, string? nonce = null, string provider = "default")
        {
            try
            {
                var oidcConfig = await GetOidcProviderWithSecret(provider);
                ...
                // First, decode without validation to see the claims
                var tokenNoValidation = tokenHandler.ReadToken(idToken) as JwtSecurityToken;
                var claimsNoValidation = tokenNoValidation?.Claims.ToDictionary(c => c.Type, c => c.Value);
                ...
                var oidcConfigDiscovery = await configManager.GetConfigurationAsync();

                var validationParameters = new TokenValidationParameters
                {
                    ValidateIssuer = true,
                    ValidIssuers = new[] { oidcConfigDiscovery.Issuer },
                    ValidateAudience = true,
                    ValidAudience = oidcConfig.ClientId,
                    ValidateLifetime = true,
                    IssuerSigningKeys = oidcConfigDiscovery.SigningKeys,
                    ClockSkew = TimeSpan.FromMinutes(5)
                };
                ...
                try
                {
                    ...
                    var tokenNonce = principal.FindFirst("nonce")?.Value;
                    var nonceValid = string.IsNullOrEmpty(nonce) || (tokenNonce == nonce);

                    _logger.LogInformation("OIDC Debug: Token validation successful - Nonce match: {nonceMatch}", nonceValid);

                    return Ok(new
                    {
                        isValid = true,
                        issuer = oidcConfigDiscovery.Issuer,
                        audience = oidcConfig.ClientId,
                        audience_match = true,
                        claims = claimsNoValidation,
                        nonce = new
                        {
                            expected = nonce,
                            actual = tokenNonce,
                        ...
                        expiresAt = tokenNoValidation?.ValidTo,
                        isExpired = tokenNoValidation?.ValidTo < DateTime.UtcNow
                    });
                }
                catch (SecurityTokenException ex)
                {
                    _logger.LogWarning(ex, "OIDC Debug: Token validation failed");
                    return Ok(new
                    {
                        isValid = false,
            ...
            {
                _logger.LogError(ex, "OIDC Debug: Token validation check failed");
                return BadRequest(new { error = ex.Message, exceptionType = ex.GetType().Name });
            }
        }
